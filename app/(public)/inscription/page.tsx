...
export default function InscriptionPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Créer le doc Firestore via client SDK (pas Admin)
      await setDoc(doc(db, 'mpl_users', cred.user.uid), {
        email, prenom, nom,
        plan: null, billing: null,
        subscriptionStatus: null, trialEnd: null,
      ...
      const msgs: Record<string, string> = {
       ...
       h/weak-password': 'Mot de passe trop faible.',
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12 bg-gray-50">
      <div className="bg-white rounded-2xl border border-gray-200 p-6 sm:p-8 w-full max-w-md shadow-sm">
        ...
        <div className="flex items-center gap-2 mb-6">
          <div c="flex-1 h-1.5 bg-orange-600 rounded-full" />
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="text" value={nom} onChange={e => setNom(e.target.value)} required placeholder="Dupont"
                className="w-full borer border-gray-300 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500"/>
            </div>
          </div>
          <div>
          ...
          <div className="flex items-start gap-2.5 pt-1">
            <inpu type="checkbox" id="cgu" checked={cgu} onChange={e => setCgu(e.target.checked)} className="mt-0.5 flex-shrink-0"/>
          ...
          <button type="submit" disabled={loading}
            className="w-full bg-orange-600 hover:bg-orange-700 disabled:opacity-60 text-white font-semibold py-3.5 rounded-xl text-sm">
            {loading ? 'Création...' : 'Continuer vers le choix du plan →'}
          </button>
        </form>
        <p className="text-center text-sm text-gray-500 mt-5">
          Déjà un compte ? <Link href="/connexion" className="text-orange-600 font-semibold">Se connecter</Link>
        </p>
   iv>
    </div>
  )
