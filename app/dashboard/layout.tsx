...
function TrialBanner({ mplUser }: { mplUser: any }) {
  if (!mplUser?.trialEnd || mplUser.subscriptionStatus !== 'trialing') return null
  const trialEnd = new Date(mplUser.trialEnd)
  const daysLeft = Math.max(0, Math.ceil((trialEnd.getTime() - Date.now()) / (1000 * 60 * 60 * 24)))
  if (daysLeft <= 0) return null
  return (
    <div className="mx-3 mb-3 bg-gradient-to-r from-orange-500 to-orange-600 rounded-xl p-3 text-white">
      <p className="text-xs font-bold mb-0.5">🎁 Essai gratuit</p>
      <p className="text-xs opacity-90">{daysLeft} jour{daysLeft > 1 ? 's' : ''} restant{daysLeft > 1 ? 's' : ''}</p>
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const initials = `${mplUser?.prenom?.[0] || ''}${mplUser?.nom?.[0] || ''}`.toUpperCase() || 'U'
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
        ...
        <div className="p-3 border-t border-gray-100">
          <div className="flex items-center gap-3 px-2 py-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-gray-900 truncate">{mplUser?.prenom} {mplUser?.nom}</p>
              <span className={`text-xs px-1.5 py-0.5 rounded font-bold ${mplUser?.plan ? 'bg-orange-100 text-orange-700' : 'bg-gray-100 text-gray-500'}`}>
                {planBadge}
              </span>
            </div>
      ...
      {/* ── MOBILE BOTTOM NAV ── */}
      <div className="md:hidden fixed bottom-0 inset-x-0 bg-white border-t border-gray-100 z-40 flex safe-bottom">
        {BOTTOM_NAV.map(item => {
          const active = pathname === item.href
          return (
            <Link key={item.href} href={item.href}
              className={`flex-1 flex flex-col items-center justify-center py-2.5 gap-0.5 transition-colors ${active ? 'text-orange-600' : 'text-gray-500'}`}>
                ...
                {NAV.map(item => {
                  ...
                  return (
                    <Link key={item.href} href={item.href} onClick={() => setMoreOpen(false)}
                      className={`flex flex-col items-center gap-1.5 p-3 rounded-xl text-center transition-colors ${active ? 'bg-orange-50 text-orange-700' : 'text-gray-600 hover:bg-gray-50'}`}>
